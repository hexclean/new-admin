  ...
  var CosticFramework = (function() {
    ...
    function animateRoundedProgress(rpb) {
      if (!$(rpb).hasClass("animated")) {
        $(rpb).css(
          "stroke-dashoffset",
          358.141563 - (358.141563 / 100) * $(rpb).attr("aria-valuenow")
        );
    ...
    function quickBarConfigure() {
      ...
      $(".ms-configure-qa").on("click", function(e) {
        ...
        stateSort = $(".ms-quick-bar-list").hasClass("ms-qa-configure-mode")
          ? "enable"
          : "disable";
    ...
    function notesMemberAppend() {
      $(".ms-members-list").on("click", "a", function() {
        var memberSrc = $(this)
          ...
          .attr("src");
        $(this)
          .closest(".ms-card-footer")
          .prev()
          .find(".ms-note-members")
          .append(
            '<li class="ms-deletable"> <img src="' +
              memberSrc +
              '" alt="member"> </li>'
          );
      });
      ...
      $(".ms-add-task-block").on("submit", function(e) {
        ...
        $(".ms-quickbar-container .ms-todo-list").prepend(
          '<div class="ms-card ms-qa-card ms-deletable"> <div class="ms-card-header clearfix">' +
            ...
            $("#task-block").val() +
            '</h6> <button data-toggle="tooltip" data-placement="left" title="Add a Task to this block" class="ms-add-task-to-block ms-btn-icon float-right">' +
    ...
    function confirmTask() {
      $(".ms-todo-list").on("submit", ".ms-confirm-task-form", function(e) {
        ...
        $(this).replaceWith(
          "<span>" +
            taskInput.val() +
            '</span><button class="close"><i class="flaticon-trash ms-delete-trigger"> </i></button>'
        );
      });
    }
